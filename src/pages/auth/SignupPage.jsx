...
export default function SignupPage({ onBack, onDone, onPrivacy, onHome }) {
  ...
  const validate = () => {
    ...
    if (!/^(?=.*[A-Za-z])(?=.*\d)(?=.*[\W_]).{8,}$/.test(password))
      nextErrors.pw = "비밀번호는 영문, 숫자, 특수문자를 포함해 8자 이상이어야 합니다.";
          ...
          <div className="social-login-stack auth-signup-social">
            {[
              ...
              <button
                ...
                className={`social-login-button ${social.className}`}
              >
                <span className={`social-login-icon ${social.className}`} aria-hidden="true">
                  {social.icon}
                </span>
