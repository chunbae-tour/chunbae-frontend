...
export function PayChargePage({ onBack, onDone, showToast }) {
  ...
  const customAmountError = isDirectInput && customAmount
  ...
  const flowSteps = [
    ...
    { label: "결제 수단 선택", done: Boolean(selected && method), active: Boolean(selected && !method) },
  ...
  const handleCharge = async () => {
    ...
    if (customAmountError) { showToast(customAmountError); return; }
    if (!method) { showToast("결제 수단을 선택해주세요!"); return; }
    setCharging(true);
    setError("");
    ...
    <div style={S.screen} className="web-payment-page">
      ...
      <div style={S.scrollArea} className="web-detail-scroll payment-charge-scroll">
        ...
        <div className="web-payment-side">
          ...
          <div className="charge-flow-summary">
            ...
            {flowSteps.map((step, index) => (
              <div key={step.label} className={`flow-step ${step.done ? "done" : ""} ${step.active ? "active" : ""}`}>
        ...
        <div className="web-payment-section web-payment-amount-section" style={{ padding: "0 16px 16px" }}>
          ...
          <div className="web-payment-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 10 }}>
            {amounts.map(a => (
              <button key={a.value} type="button" className={`payment-amount-card ${selected?.value === a.value && !isDirectInput ? "selected" : ""}`} onClick={() => { setIsDirectInput(false); setCustomAmount(""); setSelected(a); }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                const nextValue = customAmountValue >= MIN_CHARGE_AMOUNT ? customAmountValue : MIN_CHARGE_AMOUNT;
          ...
          {isDirectInput && (
            <div className="payment-direct-input-box">
              ...
              <div className="payment-direct-input-row">
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setSelected(Number.isFinite(value) && value > 0 ? {
                      value,
              ...
              <p className={customAmountError ? "invalid" : ""}>
                {customAmountError || `${formatYeopjeon(toYeopjeon(customAmountValue || 0))} 충전 예정`}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="web-payment-section web-payment-submit payment-summary-card" style={{ padding: "0 16px 32px" }}>
          {selected && (
      ...
      <div className="payment-bottom-cta">
        <div className="payment-bottom-copy">
          <strong>{selected ? `${selected.price} 충전하기` : "충전할 금액을 먼저 선택해주세요"}</strong>
          <span>{selected ? `${formatYeopjeon(selected.coinAmount)}이 충전돼요${method ? ` · ${method}` : " · 결제 수단을 선택해주세요"}` : "금액과 결제 수단을 선택하면 결제를 진행할 수 있어요."}</span>
        </div>
        <button type="button" className={`payment-primary-action payment-bottom-button ${!canPay ? "disabled" : ""}`} disabled={!canPay} onClick={handleCharge}>
          {charging ? (settlementMessage ? "충전 반영 확인 중..." : "결제창 여는 중...") : "결제하기"}
        </button>
      </div>
    </div>
  );
