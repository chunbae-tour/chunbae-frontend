...
function formatOperatingHours(hours = "") {
  ...
  return normalized
    ...
    .map((item, index) => {
      ...
      const month = item.match(
        /^((?:\d{1,2}월(?:~\d{1,2}월)?)(?:\/\d{1,2}월(?:~\d{1,2}월)?)*)\s*(.*)$/,
      );
      if (month) {
...
function mergePlaceDetailWithImageFallback(previousPlace, nextPlace) {
  ...
  const previousImageUrls = Array.isArray(previous?.imageUrls)
    ? previous.imageUrls.filter(Boolean)
    : [];
            ...
            return (
              ...
              >
                ...
                <span>
                  ...
                  <em className={closed ? "closed" : ""}>
                    {closed ? "마감" : getCompanionPeopleText(post)}
                  </em>
                </span>
              </button>
            );
...
export default function PlaceDetailPage({
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      setReviews((prev) => [
        savedReview,
        ...prev.filter((item) => String(item.id) !== String(savedReview.id)),
      ]);
      setDetail((prev) =>
        prev
          ? {
  ...
  const primaryActions = currentPlace
    ? [
        ...
        },
        {
          ...
          tone: "companion",
          onClick:
            onCompanionWrite || (() => showToast("동행 모집글 작성 화면으로 연결 예정입니다.")),
        },
      ]
    : [];
  ...
  return (
    <div style={S.screen} className="web-place-detail">
          ...
          <div className="web-place-layout">
            <aside className="place-detail-sidebar">
              <button
                ...
                style={{
                  "--place-hero-image": heroImage ? `url("${heroImage}")` : undefined,
                  height: 200,
                  background: "#F0EBE0",
                  display: "flex",
                  alignItems: "center",
                  ...
                  <div>
                    ...
                    <strong>
                      {operatingHourLines.length > 0
                        ? `${operatingHourLines.length}개`
                        : "정보 없음"}
                    </strong>
                  </div>
                  <button type="button" onClick={onDirection}>
                    길찾기
                  </button>
                </div>
              </div>
              <PlaceCompanionSidebarPanel
                ...
                onMore={
                  onCompanionMore ||
                  (() => showToast?.("동행 게시판에서 전체 모집글을 확인해주세요."))
                }
                onWrite={
                  onCompanionWrite ||
            ...
            <div className="web-place-content" style={{ padding: 20 }}>
                ...
                <div className="place-action-grid">
                  {primaryActions.map((action) => (
                    <button
                      key={action.label}
                      type="button"
                      className={`place-action-card ${action.tone}`}
              ...
              {tab === "소개" ? (
                <div>
                  <section className="place-readable-section">
                    ...
                    {descriptionExpanded && (
                      <p>{currentPlace.desc || "장소 소개가 아직 준비되지 않았습니다."}</p>
                    )}
                    {currentPlace.desc && (
                      <button
                        type="button"
                        className="place-description-toggle"
                    ...
                    {operatingHourLines.length === 0 ? (
                      ...
                      <div className="place-hour-table">
                        {operatingHourLines.map((line, index) => (
                          <div
                            key={`${line.label}-${line.value}-${index}`}
                            className="place-hour-row"
                          >
                      ...
                      <div className="place-recommendation-grid">
                        {recommendations.map((item, index) => (
                          <div
                            ...
                            className="place-mini-place-card"
                          >
                            <strong>{item.name}</strong>
                            <span>
                              {item.type} · {item.dist || "근처"}
                            </span>
                            <small>
                              {item.addr || item.desc || "상세 정보는 장소 목록에서 확인하세요."}
                            </small>
                          </div>
                        ))}
                      </div>
                      ...
                      {nearbyCategoryStatus === "success" && nearbyCategoryPlaces.length > 0 && (
                        <div className="place-external-list">
                          {nearbyCategoryPlaces.map((item) => (
                            <div
                              key={item.id ?? item.placeId ?? `${item.name}-${item.addr}`}
                              className="place-external-card"
                            >
                              <strong>{item.name}</strong>
                      ...
                      {isMarket && (
                        <div className="place-pay-flow">
                          <div>
                            <span>엽전 결제 준비</span>
                            <strong>
                              현장에서 가게 QR을 스캔하면 금액 입력과 상인 승인 후 결제가
                              완료됩니다.
                            </strong>
                          </div>
                          <button
                            type="button"
                            onClick={() =>
                              showToast("현장 QR 스캔 화면은 가게 QR을 촬영할 때 사용합니다.")
                            }
                          >
                            현장 QR 안내
                          </button>
                          ...
                          <button
                            ...
                            onClick={() =>
                              onShopClick?.({
                                ...
                                place: currentPlace,
                                placeName: currentPlace.name,
                                marketName: store.marketName || currentPlace.name,
                              })
                            }
                          >
                            🏪 {store.name}
                            ...
                            <span style={{ fontSize: 14, color: COLORS.textMuted }}>
                              {store.menu || store.description || "상점 정보"}
                            </span>
                            <br />
                            {store.verified && (
                ...
                <div>
                  {place?.reviewIntent && (
                    <div className="place-review-intent">
                      ...
                      <strong>
                        {place.shopName || currentPlace.name} ·{" "}
                        {place.paidAmount || "결제 금액 확인 중"}
                      </strong>
                  ...
                  <button
                    type="button"
                    className="place-review-open-button"
                      ...
                      <div className="place-review-form-actions">
                        <label>
                          ...
                          <input
                            ...
                            accept="image/*"
                            onChange={(event) =>
                              setReviewPhotoName(event.target.files?.[0]?.name || "")
                            }
                          />
                        </label>
                  ...
                  {reviews.length === 0 ? (
                    ...
                    reviews.map((r) => (
                      ...
                      >
                        ...
                        <div style={{ color: "#E8A020", fontSize: 14, marginBottom: 4 }}>
                          {"★".repeat(r.rating)}
                        </div>
              ...
              <div className="web-action-row" style={{ display: "flex", gap: 10, marginTop: 24 }}>
                <div
                  onClick={() => showToast("장소 정보 신고 API가 아직 연결되지 않았습니다.")}
                  style={{
                    flex: 1,
      ...
      {imageModalOpen && heroImage && (
        <div
          className="place-image-modal"
          role="dialog"
          aria-modal="true"
          aria-label={`${currentPlace?.name || "장소"} 대표 이미지`}
          onMouseDown={() => setImageModalOpen(false)}
        >
          <div
            className="place-image-modal-content"
            onMouseDown={(event) => event.stopPropagation()}
          >
            <button type="button" onClick={() => setImageModalOpen(false)} aria-label="이미지 닫기">
              ×
            </button>
