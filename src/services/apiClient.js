...
export function isAuthError(error) {
  return (
    ...
    String(error?.code || "").startsWith("AUTH_")
  );
}
...
export function getApiErrorHint(error) {
  ...
  if (error.code === "NETWORK_ERROR")
    return "백엔드 서버 실행 상태, CORS 허용 origin, VITE_API_BASE_URL을 확인하세요.";
  ...
  if (error.status === 404)
    return "프론트 endpoint와 백엔드 Controller 경로가 일치하는지 확인하세요.";
  if (error.status === 429)
...
export async function apiRequest(
  path,
  { method = "GET", auth = false, role, headers = {}, body } = {},
) {
  ...
  try {
    ...
    throw new ApiClientError("백엔드 서버에 연결하지 못했습니다.", "NETWORK_ERROR", 0, {
      method,
      url,
      cause: error,
    });
...
export async function apiFormRequest(
  path,
  { method = "POST", auth = false, role, headers = {}, formData } = {},
) {
  const token = getAccessToken(role);
  const url = buildApiUrl(path);
