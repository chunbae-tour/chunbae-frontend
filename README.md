...
| 엽전 충전 · 결제 수단 |
|---|
| ![결제](public/screenshots/payment.png) |

---

## 📝 프로젝트 노트

**백엔드 개발자 관점에서 직접 구현**한 프론트엔드입니다. 프론트 전문 경험이 깊진 않지만, 팀원들과 제가 설계한 백엔드 API에 맞춰 화면·연동 구조를 설계하고 AI 페어 프로그래밍(바이브 코딩)으로 구현을 가속했습니다. 
API 계약·결제 흐름·실시간 통신 등 핵심 설계 판단은 직접 내렸습니다.
...
단순히 AI에게 코드를 요청하는 것에서 나아가, AI가 프로젝트 맥락을 정확히 파악할 수 있도록 **컨텍스트 문서를 체계적으로 설계**했습니다. API 계약서·화면-API 매핑·구현 정책 등을 `docs/`에 정리해 AI의 환각(hallucination)을 줄이고, `AGENTS.md`로 에이전트 행동 기준을 명시해 반복 수정 비용을 최소화했습니다. 검토·판단은 직접 수행하는 **인간-AI 협업 워크플로우**를 구축한 경험입니다.
...
- **점진적 API 교체** — 기존 mock UI를 유지한 채 실제 API 연동으로 단계적으로 전환하는 구조입니다. 
API 명세에 없는 필드는 임의로 추측하지 않습니다.
- **결제 이원화** — 엽전 충전은 PortOne SDK로 프론트에서 결제창을 호출하고, 가게 QR 결제는 백엔드 주도로 처리합니다. 
결제 수단별 흐름을 분리했습니다.
- **PWA 채택** — 1달 프로젝트 일정상 앱 스토어 심사 기간·비용이 현실적으로 불가했습니다. 
서비스 실용성이 검증되면 네이티브 앱으로 전환할 수 있도록 확장 가능성을 열어뒀습니다.
